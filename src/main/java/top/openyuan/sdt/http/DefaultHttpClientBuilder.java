...
import top.openyuan.sdt.config.Config;
import top.openyuan.sdt.http.adapter.okhttp.OkHttpClientAdapter;
import top.openyuan.sdt.http.adapter.okhttp.OkHttpMultiDomainInterceptor;
...
public class DefaultHttpClientBuilder
  ...
  // 连接的 Keep-Alive 空闲时长为7秒, 合利宝支付服务器目前是8秒
  ...
  public DefaultHttpClientBuilder config(Config heliPayConfig) {
    requireNonNull(heliPayConfig);
    this.credential = heliPayConfig.createCredential();
    this.validator = heliPayConfig.createValidator();
    return this;
  }
